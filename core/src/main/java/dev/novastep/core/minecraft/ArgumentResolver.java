...
public class ArgumentResolver {
    ...
    public List<String> resolveGameArgs() {
        ...
        if (versionInfo.arguments != null && versionInfo.arguments.game != null) {
            ...
            for (String a : versionInfo.minecraftArguments.split(" ")) {
                if (!a.isBlank())
                    args.add(substitute(a));
            }
        }

        if (quickPlay && quickPlayMode != null && quickPlayValue != null) {
            boolean alreadyPresent = args.stream().anyMatch(a -> a.startsWith("--quickPlay"));
            if (!alreadyPresent) {
    ...
    private void processConditionalArg(JsonObject obj, List<String> out, boolean isGame) {
        if (obj.has("rules") && !evaluateRulesJson(obj.getAsJsonArray("rules")))
            return;
        if (!obj.has("value"))
            return;
    ...
    private void processConditionalArgMap(Map<String, Object> map, List<String> out, boolean isGame) {
        if (map.containsKey("rules")) {
            ...
            if (rulesObj instanceof List<?> rules
                    && !evaluateRulesList((List<Object>) rules))
                return;
        }
    ...
    private boolean evaluateFeaturesMap(Map<String, Object> features) {
        ...
        if (features.containsKey("is_quick_play_singleplayer"))
            return "singleplayer".equals(quickPlayMode);
        if (features.containsKey("is_quick_play_multiplayer"))
            return "multiplayer".equals(quickPlayMode);
        if (features.containsKey("is_quick_play_realms"))
            return "realms".equals(quickPlayMode);
        if (features.containsKey("has_quick_plays_support"))
            return quickPlay;
        return true;
    }
    ...
    private boolean shouldSkipGameArg(String arg) {
        if ("--demo".equals(arg) && !demo)
            return true;
        if (QUICKPLAY_ARGS.contains(arg) && !quickPlay)
            return true;
        return false;
    }
