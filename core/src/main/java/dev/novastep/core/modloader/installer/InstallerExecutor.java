    ...
    private static void downloadProfileLibraries(
        ...
        for (InstallProfile.Library lib : profile.libraries) {
            ...
            try {
                ...
                CoreLogger.get().warn(LOG, "[" + sessionId + "] Profile lib download failed ["
                        + art.path + "]: " + e.getMessage());
            }
        }
    }
            ...
            for (String dep : processor.classpath) {
                ...
                if (!Files.exists(depJar)) {
                    try {
                        ...
                        CoreLogger.get().warn(LOG, "[" + sessionId + "] Classpath dep not available: " + depJar);
                        continue;
                    }
                }
    ...
    private static Path downloadJar(
        ...
        for (String repo : repos) {
            ...
            try {
                ...
                CoreLogger.get().debug(LOG, "[" + sessionId + "] Repo failed [" + repo + "]: " + e.getMessage());
            }
        }
