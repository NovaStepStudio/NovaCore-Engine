...
public final class DownloadResult {
    ...
    private DownloadResult(DownloadTask task, boolean success, String error, long bytesWritten, boolean skipped, boolean sha1Passed) {
        this.task = task;
    ...
    public String toString() {
        if (success && skipped) return String.format("DownloadResult{SKIPPED, file='%s'}", task.name);
        if (success) return String.format("DownloadResult{OK, file='%s', bytes=%d}", task.name, bytesWritten);
