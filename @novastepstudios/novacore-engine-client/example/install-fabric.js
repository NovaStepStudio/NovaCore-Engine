...
console.log('Instalando Minecraft con Fabric...\n');
    ...
    await client.install(
        ...
        },
        {
            onStart: (totalFiles, totalBytes) => {
                ...
                console.log(`Fabric Setup: ${totalFiles} archivos | Tamano: ${sizeMb} MB\n`);
            },
            ...
            onModule: ({ module, status }) => {
                if (status === 'completed') {
                    console.log(`\nOK: Modulo [${module.toUpperCase()}] terminado.`);
                }
            },
            onComplete: (version, modloader) => {
                console.log(`\n\nFABRIC INSTALADO: ${version} (${modloader})`);
                console.log(`Listo para jugar.\n`);
            },
            onError: (reason) => {
                console.error('\n\nError en la instalacion de Fabric:', reason);
            },
        },
        1200_000
    );
